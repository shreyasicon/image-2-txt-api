...
- **Nginx for ZAP:** `.github/nginx-zap.conf` adds `try_files` for the Next.js export (fewer 404s), plus `favicon.ico` / `robots.txt` responses so probes do not return 404. It sets a **strict Content-Security-Policy** (explicit API/Cognito/Unsplash/Vercel hosts; no `https:` scheme wildcard; no `unsafe-eval`). If you change `NEXT_PUBLIC_*` API bases or Cognito region, update the CSP in **nginx-zap.conf** and **webapp/scalable/amplify.yml** to match.
